...
function approved_fxn() {
    ...
    const data = ref(db, "req/ct/la/approved/" + "21185");
...
function declined_fxn() {
    const db = getDatabase();
    const data = ref(db, "req/ct/la/declined/" + "21185");
...
function pending_fxn() {
    ...
    const db = getDatabase();
    const data = ref(db, "req/ct/la/pending/" + "21185");
...
key.addEventListener('click', (e) => {
    if (e.target.className == 'doing-accept') {
        ...
        accept_ct(reg_no_text, from_text, to_text, reason_text, address_text, mobile_no_text)
        ...
        declined_ct(reg_no_text, from_text, to_text, reason_text, address_text, mobile_no_text)
    }


})

function accept_ct(reg_no, from, to, reason, address, mobile_no) {
    const db = getDatabase();
    var today = new Date();
    set(ref(db, 'req/ct/la/approved/' + reg_no + '/' + from + '_' + to), {
        from: from,
        to: to,
        reason: reason,
        address: address,
    ...
    set(ref(db, 'req/hod/la/pending/' + reg_no + '/' + from + '_' + to), {
        from: from,
        to: to,
        reason: reason,
        address: address,
    ...
    remove(ref(db, 'req/ct/la/pending/' + reg_no + '/' + from + '_' + to), {})
    // req_fwd(reg_no, from, to, reason, address, mobile_no)
}

function declined_ct(reg_no, from, to, reason, address, mobile_no) {
    const db = getDatabase();
    ...
    set(ref(db, 'req/ct/la/declined/' + reg_no + '/' + from + '_' + to), {
        from: from,
        to: to,
        reason: reason,
        address: address,
